...
{
    /// <summary>
    /// An <see cref="ErrorStore" /> implementation that uses Oracle as its backing store.
    /// </summary>
    public sealed class OracleErrorStore : ErrorStore
    {
        ...
        /// <summary>
        /// Creates a new instance of <see cref="OracleErrorStore" /> with the specified connection string.
...
Select * 
 ...
 Where lower(GUID) = :guid");
        ...
        protected override async Task<Error> GetErrorAsync(Guid guid)
        {
            ...
            using (var c = GetConnection())
            {                
                sqlError = await c.QueryFirstOrDefaultAsync<Error>(SqlGetError, new { guid = this.GuidToSting(guid) }).ConfigureAwait(false);
            }
...
Select Count(*) 
 ...
 Where DeletionDate Is Null
   ...
   And CreationDate > :since");
        ...
        private IEnumerable<string> GuidsToStings(IEnumerable<Guid> targets)
        {
