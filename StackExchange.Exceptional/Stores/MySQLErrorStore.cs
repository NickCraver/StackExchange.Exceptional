...
namespace StackExchange.Exceptional.Stores
{
    ...
    public sealed class MySQLErrorStore : ErrorStore
    {
        ...
        public MySQLErrorStore(string connectionString, int displayCount = DefaultDisplayCount, int rollupSeconds = DefaultRollupSeconds) : base(rollupSeconds)
        {
        ...
        protected override void LogError(Error error)
        {
            using (var c = GetConnection())
            {
                if (RollupThreshold.HasValue && error.ErrorHash.HasValue)
                {
                    ...
                    var existingException = c.Query<Error>(@"Select * from Exceptions Where ErrorHash = @ErrorHash
